...
import { PageTransition, FadeIn, StaggerContainer, StaggerItem, AnimatedNumber } from "@/components/ui/motion";
    ...
    <PageTransition className="space-y-8">
      ...
      {!hasPackages ? (
        ...
        <>
                  ...
                  <div className="flex flex-wrap gap-2 w-full">
                    {pills.map((p) => (
                      <Link key={p.label} href={p.href}>
                        <button className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-card px-3.5 py-1.5 text-sm transition-all duration-200 hover:shadow-md hover:border-primary/30 hover:bg-accent/50 hover:scale-105 cursor-pointer group">
          ...
          ].filter(s => s.items.length > 0).map((section, sIdx) => {
            const Icon = section.icon;
            return (
              <FadeIn key={section.key} delay={0.15 + sIdx * 0.08}>
                <section>
